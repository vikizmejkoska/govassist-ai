...
export function ServiceDetailsPage() {
  ...
  const draftKey = `request_draft_${serviceId ?? 'unknown'}`;

  const location = useLocation();
  const locationState = location.state as { reuseTitle?: string; reuseDescription?: string } | null;
  ...
  useEffect(() => {
    ...
    void servicesApi
        ...
        .then((item) => {
          ...
          const savedDraft = localStorage.getItem(`request_draft_${serviceId}`);
          if (savedDraft) {
            const parsed = JSON.parse(savedDraft) as { title: string; description: string };
            setValues(parsed);
            setDraftRestored(true);
          } else {
            setValues({
              title: locationState?.reuseTitle ?? item.title,
              description: locationState?.reuseDescription ?? '',
            });
          }
  ...
  useEffect(() => {
    if (!serviceId || (!values.title && !values.description)) {
      return;
    }
    localStorage.setItem(draftKey, JSON.stringify(values));
  }, [values, serviceId, draftKey]);
